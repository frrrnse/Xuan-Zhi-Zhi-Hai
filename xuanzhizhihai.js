...
if (!isAnyPhotoScattering()) {
  ...
  for (let d of displays) {
    if (d.phase === PHASE.CONVERGING || d.phase === PHASE.DISPLAYED ||
        (d.phase === PHASE.SCATTERING && millis() - d.stateStart < C.scatterMs * 0.3)) {
      d.drawNickname();
    }
  }
...
getTargetParticleCount() {
  return C.photoParticleTargets[0];  
}
...
releaseParticles() {
  ...
  drawPhotoOverlay() {
    ...
    let alpha = 0;
    if (this.phase === PHASE.CONVERGING) {
      let photoFade = easeOutQuad(max(0, (this.convergeProgress - 0.5) / 0.5));
      alpha = 255 * photoFade;
    } else if (this.phase === PHASE.DISPLAYED) {
      alpha = 255;
    } else if (this.phase === PHASE.SCATTERING) {
      let sp = (millis() - this.stateStart) / C.scatterMs;
      if (sp < 0.3) {
        alpha = 255 * (1 - easeOutQuad(sp / 0.3));
      }
    }
...
function updList() {
  ...
  if (waitPool.length > 0) {
    ...
    for (let d of show) {
      ...
      let nickPool = d.nickname ? `昵称：${d.nickname}` : '匿名';
      idT.html(`${uploadMark}${starMark}#${String(d.id).slice(-6)} 等待中 (已展${showCount}次) ${nickPool}`);
      idT.parent(info);
